...
void KeyFrame::ReplaceMapCuboidMatch(const size_t &idx, MapCuboid* pMC)
{
    mvpMapCuboid[idx]=pMC;
}

void KeyFrame::EraseMapCuboidMatch(const size_t &idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
    mvpMapCuboid[idx]=static_cast<MapCuboid*>(NULL);
}
...
cv::Mat KeyFrame::ComputePlaneCenterToWorld(const int &idx) 
{
    cv::Mat temp;
    cv::transpose(Tcw, temp);
    return temp*mvPlaneCentroid[idx];
}
